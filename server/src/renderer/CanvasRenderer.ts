...
export class CanvasRenderer {
  ...
  public clearAnimationStates(): void {
    ...
    // Clear background states when clearing animations
    BackgroundFactory.clearAllBackgrounds();
  }
  ...
  private renderBackground(ctx: CanvasRenderingContext2D, template: Template, width: number, height: number): void {
    ...
    // Update background animation state if not solid
    if (backgroundConfig.type !== 'solid') {
      const currentTime = Date.now();
      const state = background.getState?.() || {};
      const lastUpdate = state.lastUpdate || (currentTime - 16);
  ...
  // Particle rendering is now handled by individual background classes

  // Matrix rendering is now handled by MatrixBackground class

  // Pipes rendering is now handled by PipesBackground class

  // Fish tank rendering is now handled by FishTankBackground class
